...
exports.deleteUserAccount = exports.updateSoldierAssignment = exports.createSoldierFromUser = exports.createUserFromSoldier = exports.processGoogleFormSubmission = exports.setCustomClaims = void 0;
...
exports.setCustomClaims = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (requesterClaims.role !== "admin" &&
            ...
            throw new functions.https.HttpsError("permission-denied", "אין הרשאה לשנות תפקידים");
        }
        // עדכון Custom Claims
        ...
        throw new functions.https.HttpsError("internal", "שגיאה בעדכון התפקיד");
    }
});
// Function to handle Google Forms submissions
exports.processGoogleFormSubmission = functions.https.onRequest(async (req, res) => {
    // הגדרת CORS
...
exports.createUserFromSoldier = functions.firestore
...
exports.createSoldierFromUser = functions.firestore
...
exports.updateSoldierAssignment = functions.firestore
    .document("users/{userId}")
...
exports.deleteUserAccount = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (requesterClaims.role !== "admin" &&
            requesterClaims.role !== "mefaked_pluga") {
            throw new functions.https.HttpsError("permission-denied", "אין הרשאה להסרת משתמשים מהמערכת");
        }
        // בדיקה שהמשתמש לא מנסה להסיר את עצמו
        if (uid === context.auth.uid) {
            throw new functions.https.HttpsError("invalid-argument", "לא ניתן להסיר את עצמך מהמערכת");
        }
        ...
        if (userToDeleteClaims.role === "admin" && requesterClaims.role !== "admin") {
            throw new functions.https.HttpsError("permission-denied", "רק אדמין יכול להסיר אדמין אחר");
        }
        // מחיקת רשומת החייל אם קיימת (לפני מחיקת המשתמש)
        let soldierDocId = null;
        try {
        ...
        try {
            ...
            throw new functions.https.HttpsError("internal", "שגיאה במחיקת המשתמש מ-Firebase Auth");
        }
        ...
        throw new functions.https.HttpsError("internal", "שגיאה בהסרת המשתמש מהמערכת");
    }
});
